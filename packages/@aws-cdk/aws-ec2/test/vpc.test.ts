...
describe('vpc', () => {
  describe('When creating a VPC', () => {
    describe('with the default CIDR range', () => {
      ...
      test('the Name tag is defaulted to path', () => {
        ...
        Template.fromStack(stack).
          hasResourceProperties('AWS::EC2::VPC',
            hasTags([{ Key: 'Name', Value: 'TestStack/TheVPC' }]),
          );
        Template.fromStack(stack).
          hasResourceProperties('AWS::EC2::InternetGateway',
            hasTags([{ Key: 'Name', Value: 'TestStack/TheVPC' }]),
          );
    ...
    test('with reserved subnets, any other subnets should not have cidrBlock from within reserved space', () => {
      ...
      for (let i = 3; i < 6; i++) {
        Template.fromStack(stack).hasResourceProperties('AWS::EC2::Subnet', Match.not({
          CidrBlock: `10.0.${i}.0/24`,
        }));
      }
      for (let i = 6; i < 9; i++) {
    ...
    test('with natGateway subnets defined', () => {
      ...
      for (let i = 1; i < 4; i++) {
        Template.fromStack(stack).hasResourceProperties('AWS::EC2::Subnet', hasTags([{
          Key: 'Name',
  ...
  describe('When tagging', () => {
    test('VPC propagated tags will be on subnet, IGW, routetables, NATGW', () => {
      ...
      Template.fromStack(stack).hasResourceProperties('AWS::EC2::VPC', hasTags(toCfnTags(allTags)));
      ...
      for (const resource of taggables) {
        Template.fromStack(stack).hasResourceProperties(`AWS::EC2::${resource}`, hasTags(propTags));
        Template.fromStack(stack).hasResourceProperties(`AWS::EC2::${resource}`, Match.not(hasTags(noProp)));
      }
    ...
    test('Subnet Name will propagate to route tables and NATGW', () => {
      ...
      for (const subnet of vpc.publicSubnets) {
        ...
        Template.fromStack(stack).hasResourceProperties('AWS::EC2::NatGateway', hasTags([tag]));
        Template.fromStack(stack).hasResourceProperties('AWS::EC2::RouteTable', hasTags([tag]));
      }
    ...
    test('Tags can be added after the Vpc is created with `vpc.tags.setTag(...)`', () => {
      ...
      // Template.fromStack(stack).hasResourceProperties('AWS::EC2::VPC', Match.not(hasTags([tag])));
      Tags.of(vpc).add(tag.Key, tag.Value);
      Template.fromStack(stack).hasResourceProperties('AWS::EC2::VPC', hasTags([tag]));
  ...
  describe('subnet selection', () => {
    ...
    test('fromVpcAttributes using unknown-length list tokens', () => {
      ...
      const asm = app.synth().getStackArtifact(stack.stackName);
...
function getTestStack(app?: App): Stack {
